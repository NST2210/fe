...
const PublicInfo = (props) => {
  ...
  const handleSaveEvent = async () =>{
    const nameImg = await FileApi.uploadImage(previewAvatarFile)
    console.log(nameImg);
    //call api update profile
    await UserApi.updateProfile(nameImg);
    showSucessNotification("Change Avatar", "Change avatar sucessfully!");
  }

  const showSucessNotification = (tilte, message) =>{
    const options = {
      timeOut: 3000,
      type: "success",
        ...
        <Row>
          ...
          <Col md="4">
            <div className="text-center">
              <img
                alt={props.fullName}
                src={previewAvatarUrl ? previewAvatarUrl : (userInfo.avatarUrl ? `http://127.0.0.1:8887/${userInfo.avatarUrl}` : avatar1 )}
              ...
              {/* button Upload */}
              <div className="mt-2">
                <Button color="primary" onClick={() => avatarInputFile.current.click()}> {/* current points to the mounted  avatarInputFile element*/}
